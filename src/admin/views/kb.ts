...
function renderListingBoard(listings: ListingSnapshot[]): string {
  ...
  return `<section style="margin-bottom:16px"><div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px"><div style="font-size:10px;letter-spacing:.14em;color:var(--accent)">PROPERTY INTELLIGENCE</div><span class="text-dim text-[10.5px]">${listings.length} ${listings.length === 1 ? "propiedad verificada" : "propiedades verificadas"}</span></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:9px">${listings.map((listing) => { const openHouse = `/admin/open-houses?${new URLSearchParams({ title: `Open House · ${listing.address}`, address: listing.address }).toString()}`; return `<article style="border:1px solid var(--line);background:var(--panel);padding:13px"><a href="/admin/kb/${encodeURIComponent(listing.doc.id)}/edit" style="display:block;text-decoration:none"><div style="display:flex;justify-content:space-between;gap:8px;align-items:start"><b class="text-cream" style="font-size:12px;line-height:1.35">${esc(listing.address || "Dirección por confirmar")}</b><span style="font-size:9px;letter-spacing:.06em;color:${colors[listing.status]};border:1px solid ${colors[listing.status]};padding:3px 5px;white-space:nowrap">${labels[listing.status]}</span></div>${listing.price ? `<div style="font-size:14px;color:var(--accent);font-family:'Space Grotesk';font-weight:700;margin-top:10px">${esc(listing.price)}</div>` : `<div class="text-dim" style="font-size:11px;margin-top:10px">Precio por confirmar</div>`}${listing.zone ? `<div class="text-muted" style="font-size:10.5px;margin-top:5px">${esc(listing.zone)}</div>` : ""}</a><div style="display:flex;gap:7px;flex-wrap:wrap"><a class="ghostbtn" href="${openHouse}" style="display:inline-block;margin-top:11px;padding:6px 8px;font-size:10px">Crear Open House + QR</a><a class="ghostbtn" href="/admin/marketing/${encodeURIComponent(listing.doc.id)}" style="display:inline-block;margin-top:11px;padding:6px 8px;font-size:10px">Marketing Copilot</a></div></article>`; }).join("")}</div></section>`;
}
