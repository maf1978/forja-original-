...
const SUPERPODERES_SKILL_FILE = fileURLToPath(new URL("../skills/superpoderes.md", import.meta.url));

function readSuperpoderesSkill() {
  return readFileSync(SUPERPODERES_SKILL_FILE, "utf8");
}

// Cada bot nuevo (y cada update) recibe su copia local. Así /superpoderes
// funciona aunque el agente global no esté instalado o el proyecto cambie de equipo.
function installBotSkills(dir) {
  try {
    const skillDir = join(dir, "skill");
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, "superpoderes.md"), readSuperpoderesSkill());
    return true;
  } catch {
    return false;
  }
...
// Instala las guías para el AGENTE del miembro (Claude Code o Codex) que le enseñan a usar
// el CLI y /superpoderes. Se escriben en ~/.claude/skills y ~/.codex/skills. Idempotente;
...
function installAgentSkill(flags = {}) {
  ...
  try {
    ...
    const existed = roots.some((root) => existsSync(join(root, "forja", "SKILL.md")));
    for (const root of roots) {
      const forjaDir = join(root, "forja");
      const superDir = join(root, "superpoderes");
      mkdirSync(forjaDir, { recursive: true });
      mkdirSync(superDir, { recursive: true });
      writeFileSync(join(forjaDir, "SKILL.md"), AGENT_SKILL);
      writeFileSync(join(superDir, "SKILL.md"), readSuperpoderesSkill());
    }
    if (!existed) console.log(C.dim("  \u270e guías de Forja instaladas para tu agente  \u2192  ~/.claude/skills + ~/.codex/skills"));
...
export { renderMemberConfig, stampBrandAndBrain, writeStarterConfig, select, forgeSplash, installAgentSkill, installBotSkills, readSuperpoderesSkill, parseFlags, starterOnboarding, loadCreds, saveCreds, normalizeWorkerUrl, listenLoopback, stampBotConfig, applyBusinessFlags };
